...
def generate_topic(api_key: str, category_name: str, used_titles: list) -> dict:
    ...
    prompt = f"""Придумай одну новую тему статьи для делового журнала в разделе "{category_name}".

Уже опубликованные темы (НЕ повторяй):
{used_str}

Требования:
- Заголовок конкретный и экспертный, в стиле делового издания
- НЕ используй аббревиатуру "B2B" в заголовке — пиши по сути: "корпоративные продажи", "сложные сделки", "управление компанией" и т.д.
- Тема актуальна для руководителей, собственников и топ-менеджеров
- На русском языке

Верни ТОЛЬКО JSON:
{{"title": "Заголовок статьи", "excerpt": "Краткое описание 1-2 предложения без слова B2B"}}"""
...
def generate_article_content(api_key: str, category_name: str, title: str, excerpt: str) -> dict:
    ...
    prompt = f"""Ты — редактор делового журнала btwob.ru. Напиши профессиональную статью на русском языке.
...
- Избегай шаблонного употребления "B2B" там, где можно сказать конкретнее: "корпоративный рынок", "сложные сделки", "корпоративные клиенты", "бизнес-процессы" и т.д.
